...
$groups = $admin->getRows("SELECT * FROM groups");
			...
			<div class="col-md-12">
				<div class="col-md-12">
					<table id="example" class="table table-striped table-dark table-bordered dt-responsive nowrap" style="width:100%; ">
					    ...
					    <tbody>
					    	<?php foreach ($groups as $key) { $id = $key->id; $href= "group.php?id=".$id; ?>
					       <tr>
					           <td><?=$key->groupVIp?></td>
					           <td><?=$key->groupName?></td>
					           <td><?=$key->groupCountry?></td>
					           <td><a href="<?=$href?>" class="btn btn-primary">Görüntüle</a></td>
					       </tr>
					   		<?php } ?>
					    </tbody>
					</table> <br> <hr> <br>
					<form action="process.php" method="POST" class="form">
						...
						<input type="text" name="groupName" placeholder="Group Name" required="" class="form-control">
						<input type="text" name="groupLanguage" placeholder="Group Language" required="" class="form-control">
						<p style="font-size:16px; margin-top:-10px;">Turkish - English vb.</p>
						<input type="hidden" name="token" value="<?=$token?>">
						
							<div class="checkbox-area">
								<label> <input class="form-check-input" type="checkbox" name="special"> Özel</label>
							</div>
							<div class="checkbox-area">
								<label> <input class="form-check-input" type="checkbox" name="editable"> Düzenlenebilir </label>
							</div> <button type="submit" name="addGroup" class="btn btn-success" style="float:right">Ekle</button>
							<p><i class="fas fa-info-circle"></i> Grubunuzu özel yaparsanız sadece link ile giriş yapanlar görüntüleyebilir. Grup ekleme işlemi sonrası size grubunuzun linki verilecektir.</p>
							<p><i class="fas fa-info-circle"></i> Grubunuzu düzenlenebilir yaparsanız grubunuzu görüntüleyenler ekleme yapabilir.</p>
							<p><i class="fas fa-info-circle"></i> Grubunuzu ekledikten sonra sadece size özel düzenleme linki de verileceğini unutmayın.</p><br>
					</form>
				</div>
			</div>
		</div>
