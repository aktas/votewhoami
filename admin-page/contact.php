...
$messages = $messages->getRows("SELECT * from messages");
			...
			<div class="col-md-12">
				<div class="col-md-12">
					...
					<table id="example" class="table table-striped table-dark table-bordered dt-responsive nowrap" style="width:100%; ">
					    ...
					    <tbody>
					    	<?php foreach ($messages as $key) { $id = $key->id; $href= "process.php?messagesDelete=ok&token=".$token."&id=".$id; ?>
					       <tr>
					           <td><?=$key->messagesIp?></td>
					...
					<form action="process.php" method="POST" class="form">
						...
						<button type="submit" name="sendMail" class="btn btn-success">Ekle</button>
					</form> <br>
					<br> 
				</div>
			</div>
		...
		function run(){
			...
			setTimeout(function(){
  				$.ajax({
					type: 'POST',
					url: 'process.php',
					data: 'algorithmRun=ok&token='+token,
					beforeSend: function(){
						$("#run").html('Run <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>');
					...
					success: function(result){
						...
						$("#run").html('Run');
						var audio = new Audio('sounds/beep-success.wav');
						audio.play();
					}
				});
			}, 3000);
