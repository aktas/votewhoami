...
$visitors = new \votewhoami\admin\visitors();

$visitor = $visitors->getRow("SELECT * FROM visitors WHERE id = ?",array($getId));
$ip = $visitor->visitorsIp;
$groups = $visitors->getRows("SELECT * FROM groups WHERE groupVIp = ?",array($ip));
$groupMembers = $visitors->getRows("SELECT * FROM groupMembers WHERE ip = ?",array($ip));
$messages = $visitors->getRows("SELECT * FROM messages WHERE messagesIp = ?",array($ip));
			...
			<div class="col-md-12">
				<div class="col-md-12">
					<div class="row">
						<div class="col-md-6">
							<?php 
							echo "Durum: ".$data->status." <br> ";
							echo "Ülke: ".$data->country." <br> ";
							echo "Ülke Kodu: ".$data->countryCode." <br> ";
							echo "Şehir: ".$data->regionName." <br> ";
							echo "Posta Kodu: ".$data->zip." <br> ";
							echo "Saat Dilimi: ".$data->timezone." <br> ";
							?>
						</div>
						<div class="col-md-6">
							<?php 
								echo "İnternet Sağlayıcısı: ".$data->isp." <br> ";
								echo "Firma Adı: ".$data->as."<br>";
								echo "İp: ".$ip."<br>";
								echo "Platform: ".$visitor->visitorsPlatform."<br>";
								echo "Agent: ".$visitor->visitorsAgent."<br>";
								echo "Language: ".$visitor->visitorsLanguage."<br>";
								echo "Click: ".$visitor->visitorsClick;
							?>
						</div>
						<div class="col-md-12">
							<p>Cookies:</p>
							<?php $cookies = json_decode($visitor->visitorsCookies); foreach ($cookies as $key => $value) {
								echo "<p>".$key." => ".$value;
							} ?>
						</div>
					</div>
					...
					<table id="example" class="table table-striped table-dark table-bordered dt-responsive nowrap" style="width:100%; ">
					    ...
					    <tbody>
					    	<?php foreach ($groups as $key) { $id = $key->id; ?>
					       <tr>
					          <td><?=$key->groupName?></td>
					          <td><?=$key->groupCountry?></td>
					          <td><?=$key->groupDate?></td>
					          <td><a href="group.php?id=<?=$id?>" class="btn btn-primary" >Görüntüle</a></td>
					       </tr>
					   		<?php } ?>
					    </tbody>
					</table> <br> <br>
					<h4>Group Member:</h4>
					<table id="example2" class="table table-striped table-dark table-bordered dt-responsive nowrap" style="width:100%; ">
						<thead>
					        <tr>
					            <th>Name</th>
					            <th>Characters</th>
					    ...
					    <tbody>
					    	<?php foreach ($groupMembers as $key) { $id = $key->id; ?>
					       <tr>
					           <td><?=$key->name?></td>
					...
					<h4>Messages:</h4>
					<table id="example3" class="table table-striped table-dark table-bordered dt-responsive nowrap" style="width:100%; ">
						<thead>
					        <tr>
					            <th>Name</th>
					            <th>Email</th>
					    ...
					    <tbody>
					    	<?php foreach ($messages as $key) { $id = $key->id; ?>
					       <tr>
					           <td><?=$key->messagesName?></td>
					           <td><?=$key->messagesEmail?></td>
					           <td><?=$key->messagesContent?></td>
					           <td><?=$key->messagesDate?></td>
					       </tr>
					   		<?php } ?>
					    </tbody>
					</table> <br> <br>
